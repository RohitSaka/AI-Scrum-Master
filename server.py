    ...
    try:
        ...
        for p in priorities:
            for model in available_models:
                if p in model: return model.replace("models/", "")
        if available_models: return available_models[0].replace("models/", "")
...
def get_analytics(project_key: str, creds: dict = Depends(get_jira_creds)):
    ...
    for i in issues:
        ...
        # FIXED: Added tasks array back for the Timeline UI to consume
    ...
    Tickets: {tickets}
    Return JSON: {{"executive_summary": "2 sentences on risks/health.", "risk_level": "Low/Medium/High", "key_recommendation": "Advice for PM."}}
    """
...
async def estimate_ticket(payload: dict, creds: dict = Depends(get_jira_creds)):
    return {"status": "success", "points": 0, "reason": "Not implemented"}
